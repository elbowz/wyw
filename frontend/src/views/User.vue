    ...
    <div v-if="user.id">
      <b-row align-v="center">
        ...
        <b-col class="text-right">
          <b-button
            :to="{ name: 'watched', params: { id: user.id }}"
            variant="outline-primary"
            aria-label="watched"
          >
            <b-icon icon="eye-fill" aria-label="watched"></b-icon>Watched List
          </b-button>
        </b-col>
      </b-row>

      <b-alert
        v-if="$root.store.user.id === user.id"
        class="mt-4"
        show
        fade
        dismissible
      >This is you personal profile, more information and action wil be added soon :)</b-alert>
    </div>
    <div v-if="error">
      <b-alert show="6" variant="warning" dismissible fade>{{ error }}</b-alert>
    </div>
  </div>
</template>
